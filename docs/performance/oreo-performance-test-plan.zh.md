...
| 项 | 内容 |
|---|---|
| **目标** | 回答"**一次** create / dat-to-json 消耗多少时间、CPU、内存"，得出并发上限的推导依据 |
| **负载形态** | **1 并发**，遍历文件档位：small / medium / large（各 30 次取分布） |
| **变量** | 仅文件大小。其余一切固定 |
| **关键指标** | 单次耗时（按档）· **单次内存峰值** · 内存放大系数 · CPU 时间 · 耗时对文件大小的函数形态 |
| **通过标准** | PERF-07/08/09/10 达标；**耗时随文件大小呈线性或次线性** |
| **实现** | 🟨 脚本已有（`jmx/api/p02-trade-create.jmx` + `smoke` profile + 分档 CSV），**内存指标缺采集手段** |
| **阻塞** | **OBS-02**（解析内存峰值不可观测）。当前只能出耗时结论，不能出内存结论 |
...
| **S-16** | 全容量混合负载 | [Workload Modeling](workload-modeling.zh.md) §6 全部设计容量同时施加 | **PERF-19**（所有阈值同时达标） | 唯一能暴露跨路径资源竞争的场景 |
...
| **`.dat` 样本** | small / medium / large / invalid 各若干 | 业务提供真实样本 | ⬜ **阻塞 S-01**（`data/dat/` 当前为空） |
...
| 3 | **`.dat` 真实样本缺失** | S-01 无法执行，即递进链第一环断裂 | 向业务索取各档真实样本 |
