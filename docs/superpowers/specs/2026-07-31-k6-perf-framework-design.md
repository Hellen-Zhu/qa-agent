...
- **身份模块（lib/users.js）**：系统无 token 认证，统一通过 `X-User-Id` 请求头传递身份（如 `maker@sc.com`）。身份池在 `config/` 中按角色维护（maker / checker 各若干），**权限服务端强制互斥**（2026-08-02 确认）：maker 只能发起（create / trigger-event，无 approve 权限），checker 只能裁决（approve / reject，不发起业务操作）——四眼原则由权限模型保证，不存在自批路径。场景按业务动作选角色：create/trigger-event 用 maker，checker-task API 用 checker；LIVE 池自动铺底与 E2E journey（均 P1c）必须双角色协作。错误语义 **2026-08-06 实测定稿**（推翻 2026-08-02"权限=409"的口头假设）：**权限错误 = http-403**（消息形如 "User ... does not have CHECKER permission for product=... event=..."——权限按 product 维度授予，checker 账号须覆盖用例池全部 productType）；**状态冲突 = http-400**（"Task ... is not PENDING"）；409 未被观察到。403 首先怀疑身份池配错或缺 product 权限，属配置问题而非性能问题。无 token 过期问题，soak 场景无需刷新逻辑。
...
- **lifecycle 事件数据（P1b/P1c，2026-08-02 需求评审定稿）**：
  - **状态机前置**：`create（maker）→ PENDING APPROVAL → checker 审批通过 → LIVE`，全部 trigger-event 事件只能发生在 **LIVE** trade 上。审批是独立的 checker-task API（`GET /api/v1/checker/tasks/pending`、`POST /api/v1/checker/tasks/{taskId}/approve` 与 `.../reject`；另有 bulk 端点 `bulk-approve`/`bulk-reject`——不在 P0（2026-08-03 定），但 P1c 的 seed 流水线可选用 bulk 一次批 N 笔简化 LIVE 池铺底；P0 Phase-W 压测对象是**单笔** `{taskId}/approve`/`reject`，其契约与 seed/journey 复用同一原子操作），**以 taskId 而非 tradeId 寻址**。~~自动化审批须先查 pending 清单做 tradeId→taskId 映射~~——**2026-08-05 实测推翻**：create/update 的响应 `msg` 直接携带 TaskId（"Submitted for checker approval. TaskId: CHK-…"），审批自动化零映射查询（`extractTaskId`，checker-flow/tasks.js）；pending 列表 API 降级为纯运维工具，不再是 seed 流水线依赖。approve 契约已校准（空 body POST → `code=200 + status='SUCCESS'` + `data.id` 回显）；update 契约已校准（→ `status='PENDING APPROVAL'` + 新 TaskId，eventStatus=Amended，**每次 update 消耗一个 LIVE id**——消耗池 exactly-once 游标见 consumable-ids.js）。seed 流水线（src/seed/：create 停在 taskId 层收割 → approve-tasks 池；create+approve 收割 LIVE id → update-ids 池）与两个测量场景（trades-update / checker-approve）已实现（P0 Phase-W 第一批，2026-08-05）。
...
- **dashboards/ 内容**（均为 JSON 进版本库）：
  ...
  2. **单板总览 dashboard（自建，`perf-trade-business.json`，日常主看板；2026-08-02 按"summary 对齐"模型重构）**——版面按**数据可信度分区**：
     ...
     - **2026-08-03 角色分层修订**（工程师+Leader 双视角，16 面板）：Live 行（19665 Performance Overview 对应物）曾加入、同日删除——stale markers 使其在跑后恒为 No data/残影，而盯盘职责本就属于 k6 web dashboard（:5665，秒级），看板专注跑后分析；归因收敛为 reason 级 Failure Attribution（topk10 精确条形图——rate() 对稀疏错误序列不可渲染的教训；row 级定位回归终端产物 summary/k6.log/result.csv）；错误时序改累计阶梯；增 **Capacity Analysis 行**（负载/吞吐/延迟三线图 + Latency-Load、Throughput-Load 两块 XY 散点——经典性能曲线，仅 ladder/stress 有意义，拐点判读手册与复核流程见容量分析方法论）；延迟面板画 SLA 参考线（值与 config/slas 手动同步）；增 **Run History 表**（Leader 跨轮回归方向：精确计数 + 近似 p95 标注）；testid 变量多选+All（全表达式正则匹配）；
...
10. **checker-task API 细节（P1c）**——~~checker 能否自批~~（2026-08-02 已确认：权限服务端强制互斥，maker 不能 approve、checker 不发起业务操作，自批路径不存在）；**错误语义已实测关案（2026-08-06）**：权限拒绝 = **http-403**（早前 409 假设作废）；状态冲突 = **http-400**（"Task ... is not PENDING (current: APPROVED)"）；两者 body 均为 `error/message/timestamp` 形态（非业务信封），409 未被观察到。裁决：**引擎无需特例**——403/400 维持 technical 归类，reason 即状态码天然二分（403=配置、400=池数据过期），k6.log 的 body 摘录承担细粒度归因；消耗池的 exactly-once 游标保证正常轮次不自造 400。剩余待确认仅 `checker/tasks/pending` 的过滤/分页参数——且 msg-TaskId 发现后该 API 已降级运维工具，优先级随之降低。
