...
10. **checker-task API 细节（P1c）**——~~checker 能否自批~~（2026-08-02 已确认：权限服务端强制互斥，maker 不能 approve、checker 不发起业务操作，自批路径不存在）；权限拒绝响应形态已确认：**HTTP 409**。仍待确认：`checker/tasks/pending` 的过滤/分页参数（tradeId→taskId 映射依赖它）；**409 是否兼作状态冲突**（对非 LIVE/已终结 trade 触发事件、并发修改同一笔）——若是，409 不能一律按现行规则归 technical：「目标 trade 状态已失效」语义上是数据问题（business，消耗性数据下 lifecycle 压测最可预期的失败模式），混进 technical 会污染性能结论。P1b 采集 409 的 body（是否含可枚举 code），届时决定：引擎对 409 开"读 body 再分类"特例，或维持 technical 但以 reason 细分（http-409-permission / http-409-state）。
