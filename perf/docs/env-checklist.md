...
- [ ] 消耗池工作流（update / approve 压测前置）：`./run.sh seed-update-pool <env> seed ITERATIONS=<计划迭代×1.3>` → `cp <run>/seed-pool.json data/worker-svc/trade/update-ids.json`（approve 同理用 seed-approve-pool → approve-tasks.json）；池子**单次有效**，每轮测量后重铺；update payload 红线——不得把 trade 挪出 PERF portfolio（update-payload.json 注释）；顺手实验：approve 一个已消费 task 一次，确认 409 的状态冲突语义（关掉 spec §11-10 悬案）
...
- [ ] X-User-Id 身份：申请 **20 个压测专用 maker 账号**（需真实 maker 角色 + PERF portfolio 建单权限——权限缺失表现为 409 而非 429），逐行替换 `users.maker` 占位（maker01..maker20）并确认在目标环境有效；**checker 同样按并发扩池（建议 20 个）**——checker-approve 压测与 seed 流水线的 approve 段都消耗 checker 限流预算，单账号会复现 maker 当初的 429
