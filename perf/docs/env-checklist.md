...
- [ ] 服务端指标串联配置（依赖现网信息）：业务 dashboard 底部补服务端资源面板（PromQL 参照现有服务端 dashboard，按 service 过滤）；板顶 "Server Metrics (backend)" 链接已就位（keepTime 自动带压测时间窗），把占位 uid `TBC-SERVER-DASHBOARD-UID` 替换为现网服务端 dashboard 真实 uid 即通
