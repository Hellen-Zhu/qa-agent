...
## 10. report.html 与 Grafana 的分工（结论媒介 vs 过程媒介）

同一轮测试有两套出口，各说各的语言，互不替代。

| 维度 | k6 report（report.html，每轮产物） | Grafana 看板 |
|---|---|---|
| 时间形态 | 整轮聚合**终值**——每指标一个数 | **时间序列**——每指标一条曲线（5s 窗口） |
| 回答的问题 | "这轮结果是什么：过没过、多少" | "过程中发生了什么、**何时**发生、和服务端哪个资源相关" |
| 数据范围 | 仅 k6 客户端视角 | 客户端 + 服务端两侧（JVM/连接池/gRPC——对减定位靠它） |
| 口径精度 | 精确整轮口径（与 summary 同源，判定级） | 曲线区为 5s 窗口趋势口径；仅对账卡精确（见 §0） |
| 交付形态 | 单文件零依赖——邮件直发、贴 Confluence、永久存档 | 需内网 Grafana 访问权限，交互式 |
| 生命周期 | 每轮冻结随 results/ 归档，跨月可查 | 受 Prometheus retention 限制——长期对比靠归档的 report/summary |

典型差别：soak 轮 p95 从 300ms 漂到 500ms——report 只给"整轮 p95=380ms"（漂移被平均掉）；
Grafana 曲线看到第 5 小时爬坡 + 同时刻 heap 抬头。**report 答不了"何时"，而"何时"常是根因线索。**

### 人群映射（完整分发链）

| 产物 | 人群 | 他们要的答案 |
|---|---|---|
| report.html | 业务方、管理层、不碰工具链的干系人 | "过了没有、数字多少"——签核会/邮件汇报直接发 |
| summary.txt/json | 测试工程师 + 机器（CI 退出码、基线对比） | **判定权威**：三分类、verdict、与基线的差 |
| 自建 Grafana 板 | 测试工程师 | 执行中盯盘（熔断前兆）、回溯找拐点、对账核对 |
| 19665 + 后端 OTel 板 | 性能工程师 + 开发（联合定位） | HTTP 分阶段耗时、RED/JVM/池——"为什么慢" |
| k6 web dashboard (:5665) | 执行者本人，仅运行中 | 实时曲线，非权威口径 |

给领导：report.html + test-plan 阶段表。给开发提缺陷：Grafana 链接（run.sh 打印的带精确时间窗
URL）+ result.csv 摘录。

**试金石：看对方拿到后的下一个动作。** 动作是"批准/汇报/排期"→ report（结论媒介，数字必须判定级）；
动作是"定位/调参/改代码"→ Grafana（诊断媒介，形状比数字重要）。发反了 = 让决策者做诊断、让工程师拿
结论猜过程。§0 的对账纪律正是把两个世界钉在一起的螺栓：数字对不上时，一切分析先停。
