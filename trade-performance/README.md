...
[测试计划](../docs/performance/oreo-performance-test-plan.zh.md)（选型依据在 §1.1）、
NFR、KPI 定义、负载建模。**本工程只回答"怎么实现"。**

> 历史：本工程起初与 JMeter 版并行对照；2026-07-29 计划采纳 k6 为主线后，
> JMeter 存量（jmx / groovy / scripts / 配套手册）已从仓库移除 ——
> 需要考古时在 git 历史 `87f124d` 之前找。2026-07-30 起 k6 是唯一实现，
> 原先的 `k6/` 子目录层随之取消（工程直接住在 `trade-performance/`）。
...
# 4. 阶梯找拐点（闭合模型）
./run.sh p02-trade-create dev ladder

# 5. 按到达率施压（开放模型）
./run.sh p02-trade-create dev arrival RATE=2

# 6. 列表接口（S-09 扇出审计 / S-10 数据量伸缩的载体）
./run.sh p05-trades-list dev smoke
./run.sh p05-trades-list dev smoke BLOTTER_PAGE_SIZE=500

# 7. E2E 前端链路（refdata 地址确认前先用 static 降级，报告须标注偏差）
...
看请求体靠 `K6_HTTP_DEBUG=full` 逐笔打印报文（仅 smoke 级核对用）。

### ⚠ 4. `open()` 的路径以**脚本文件**为基准

不是当前工作目录。数据路径都以**工程根目录**为根，每个做 `open()` 的模块自带
回到根目录的 ROOT 前缀（lib/ 下是 `../`，steps/<svc>/<domain>/ 下是 `../../../`）。
...
| **请求构造**（create-trade.js） | `K6_HTTP_DEBUG=full ./run.sh <plan> dev smoke` 看实际发出的 multipart | ⬜ 手工 |
| **端到端** | `./run.sh p02-trade-create dev smoke` | ✅ |

**纯逻辑要和 k6 API 隔离**（`lib/rows.js` 不 import 任何 `k6/*`）——
隔离开才能脱离 k6 运行时单独验证（`node --check` 或临时喂假数据跑一遍）。
...
- [ ] 服务端临时文件竞态缺陷（并发 create 报"找不到 dat"）：跟进修复；
      ...
      开关**用原名并发复测作为回归验证（机理与开关说明见
      `steps/workers/trade-management/create-trade-data.js` 的 DAT_NAME_MODE 段）
