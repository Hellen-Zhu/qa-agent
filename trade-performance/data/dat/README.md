...
体积现在由 `scripts/index-dat.py` **实测填入 `datSizeBytes` 列**。手写不了，就编不出来。
...
2. **CSV 里 5 行看起来是 5 个用例，实际是 1 个。** 覆盖度的假象比没有覆盖更危险。
...
./scripts/run.sh p02-trade-create dev baseline \
    -JcreateDataFile=data/create-trade/create-trade-invalid.csv
```
...
```bash
./scripts/index-dat.py            # 只报告
./scripts/index-dat.py --write    # 把 datSizeBytes 填成实测字节数
```

它查三件事：CSV 引用的文件存在、磁盘上的文件都被引用、字节数与声明一致
（第三条能抓到"换了文件但标签没跟着换"）。

## 目前状态

**空目录，需要业务/开发提供真实样本。** 最省事的采集方式见
[`../../HANDBOOK.zh.md` 阶段 0](../../HANDBOOK.zh.md)：在 UI 上手工建一笔 trade，
...
加之前先确认真实文件体积——`index-dat.py` 会告诉你。
