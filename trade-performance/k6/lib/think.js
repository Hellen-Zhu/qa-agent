 ...
 *
 * 对应 jmx 里 Flow Control Action + Uniform Random Timer 的组合。
 * JMeter 侧的坑（Timer 按**作用域**生效，直接挂在 fragment 下会给子树里
 * 每一个 sampler 都加延迟）在 k6 里不存在：sleep() 写在哪就停在哪。
 *
 ...
 * k6 的 group_duration **包含 sleep**（不同于 JMeter TransactionController
 * 的 includeTimers=false）。业务耗时看分步指标（summary 的"分步耗时"段）。
 *
 * ── THINK_SCALE ──
