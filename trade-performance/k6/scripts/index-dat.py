#!/usr/bin/env python3
"""
index-dat.py —— 把 data/dat/ 的实际内容与 create-trade 数据文件对账（只读，不改文件）。

  ./scripts/index-dat.py

数据源：data/create-trade/*.json（契约见 lib/rows.js：rows 键下，_ 开头的键是注释）。

══ 它检查两件事 ═══════════════════════════════════════════════════
  1. 数据引用的 .dat 在磁盘上存在        —— 缺文件会在压测中期才炸，且报错含糊
  2. 磁盘上的 .dat 都被某一条数据引用    —— 采集了却没进数据文件 = 白采集

另有一条覆盖度告警：正常路径 productType 少于 3 个时提示补齐
（成本画像需要 最便宜 / 最贵 / 最常见 三个代表，见 data/dat/README.md）。
"""
import json
import sys
    ...
    for src in sources:
        ...
        for i, row in enumerate(rows, start=1):
            ...
            if not (DAT_DIR / dat).is_file():
                errors.append(
                    f"{rel} 第 {i} 条引用的 .dat 不存在 — data/dat/{dat}  "
