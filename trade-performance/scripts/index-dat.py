#!/usr/bin/env python3
"""
index-dat.py —— 把 data/dat/ 的实际内容与 create-trade CSV 对账。

  ./scripts/index-dat.py            只报告，不改文件
  ./scripts/index-dat.py --write    把 datSizeBytes 列填成实测字节数

══ 为什么体积必须是实测值，不能手写 ═════════════════════════════════
早期版本用 data/dat/{small,medium,large}/ 三个目录 + CSV 里一个 datSize 标签。
那个结构隐含了一个错误前提：**体积是可以自由选择的自变量**。

它不是。体积是**产品结构的结果** —— 一笔 TARF 文件大，是因为它有 24 个定盘；
现实中不存在"TARF × small"这种组合。当时的 CSV 就长成了 5 行全是 FX_TRF、
只有体积不同的样子，那是一组现实中不存在的用例。

现在目录按 productType 分，体积由本脚本实测填入。手写不了，就编不出来。

══ 它检查三件事 ═══════════════════════════════════════════════════
  1. CSV 引用的 .dat 在磁盘上存在        —— 缺文件会在压测中期才炸，且报错含糊
  2. 磁盘上的 .dat 都被某一行 CSV 引用    —— 采集了却没进 CSV = 白采集
...
CSV_FILES = sorted((ROOT / "data" / "create-trade").glob("*.csv"))
...
def main() -> int:
    ...
    for csv_path in CSV_FILES:
        ...
        for i, row in enumerate(rows, start=2):
            dat = (row.get("datFile") or "").strip()
            if not dat:
                errors.append(f"{rel} 第 {i} 行 datFile 为空")
                continue
            referenced.add(dat)
            ...
            if not full.is_file():
                errors.append(
                    f"{rel} 第 {i} 行引用的 .dat 不存在 — data/dat/{dat}  "
                    f"(caseId={row.get('caseId')})"
                )
                continue

            actual = full.stat().st_size
            declared = (row.get("datSizeBytes") or "").strip()
            ...
            if declared in ("AUTO", "", "TBC"):
                ...
                errors.append(
                    f"{rel} 第 {i} 行 datSizeBytes={declared} 与实际 {actual} 不符 — "
                    f"文件换过了？重跑 --write 或确认是不是换错了文件"
                )
            elif not declared.isdigit():
                errors.append(
                    f"{rel} 第 {i} 行 datSizeBytes='{declared}' 不是数字 — "
    ...
    for orphan in sorted(on_disk - referenced):
        warnings.append(
            f"data/dat/{orphan} 在磁盘上但没有任何 CSV 行引用它 —— "
            f"采集了却没进 CSV，等于没采集"
        )
    ...
    types = set()
    for csv_path in CSV_FILES:
        if "invalid" in csv_path.name:
            continue
        for row in csv.DictReader(csv_path.open()):
            t = (row.get("productType") or "").strip()
            if t and t != "TBC":
                types.add(t)
    if len(types) < 3:
        warnings.append(
